...
export interface IOrderGoodModel
    extends Model<
        InferAttributes<IOrderGoodModel>,
        InferCreationAttributes<IOrderGoodModel>
    > {
    quantity: number
}
...
const OrderGoodModel = (ctx: IContextContainer) => {
    const OrderGood = <OrderGoodType>ctx.db.define<IOrderGoodModel>(
        'OrderGood',
        {
